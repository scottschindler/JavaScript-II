// A local community center is holding a fund gfgrasising 5k fun run and has invited 50 small businesses to make a small donation on their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event along with a small donation.
...
let total = ticketPriceTotal.reduce(function(acc,curr){
return acc + curr
...
// problem: Find all people that have more than 6 characters in their last name. Their shirts might cost more money!

let longName = runners.filter(x => x.last_name.length > 8);
...
//get the people that only donated more than $100
...
// Capture all the last name is alphabetical order

let last = runners.map(x => x.last_name);
let New = last.sort();
